...
def infix_to_postfix(expression):  # แปลงนิพจน์จาก infix เป็น postfix
    ...
    for token in tokens:
        if token.isdigit():
            ...
            token == ")"
        ):  # ถ้า token เป็นวงเล็บปิด จะดึงตัวดำเนินการออกจาก stack จนกว่าจะเจอวงเล็บเปิด
            while (
                stack and stack[-1] != "("
            ):  # ดึงตัวดำเนินการออกจาก stack จนกว่าจะเจอวงเล็บเปิด
...
def inorder(node):  # การเดินทางแบบ inorder (ซ้าย, ราก, ขวา)
    ...
    )  # เดินทางไปทางซ้ายก่อน, จากนั้นเพิ่มค่าตัวเอง, แล้วไปทางขวา
...
def preorder(node):
    ...
    )  # Preorder : พิมพ์ค่าตัวเองก่อน → ลูกซ้าย → ลูกขวา
...
def postorder(node):
    ...
    )  # Postorder : ลูกซ้าย → ลูกขวา → พิมพ์ค่าตัวเองทีหลัง
...
def evaluate_postorder(postfix):
    ...
    ops = {
        ...
        "*": operator.mul,
        "/": operator.truediv,
    }  # กำหนดฟังก์ชันสำหรับตัวดำเนินการแต่ละตัว
    for token in postfix:  # วนลูปผ่านแต่ละ token ในนิพจน์ postfix
        if token not in ops:  # ถ้า token ไม่ใช่ตัวดำเนินการ (เป็น operand)
            ...
            stack.append(
                ops[token](a, b)
            )  # ประเมินผลโดยใช้ฟังก์ชันที่ตรงกับตัวดำเนินการและเก็บผลลัพธ์ไว้ใน stack
...
def process_expression(expression):  # ฟังก์ชันหลักในการประมวลผลนิพจน์
    ...
    tree_root = build_expression_tree(
        postfix
    )  # สร้าง Binary Expression Tree จากนิพจน์ postfix
    in_ord = inorder(
        tree_root
    )  # ทำการเดินทางแบบ inorder, preorder, postorder และประเมินผล
...
with open("C:\\Users\\user\\Downloads\\lab_6 example.txt", "r") as f:
    lines = f.readlines()
