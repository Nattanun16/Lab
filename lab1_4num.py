...
def find_gcd1(m, n, a, b): #ห.ร.ม. โดยการแยกตัวประกอบแบบ naive
    ...
    for p in m_factors: #วนดูทีละตัวประกอบจากฝั่ง m
        if p in n_factors:
            if p in a_factors:
                if p in b_factors:#เช็คว่า p ตัวนี้มีอยู่ในลิสต์ตัวประกอบของ n ด้วยไหม โดยถ้ามี แปลว่า p เป็น “ตัวประกอบร่วม” อย่างน้อยหนึ่งตัว
...
def find_gcd2(m, n, a, b): #ห.ร.ม. โดยใช้ Sieve of Eratosthenes
    counter = [0]
    primes = sieve(max(m, n, a, b)) # สร้างลิสต์จำนวนเฉพาะที่ใช้ในการแยกตัวประกอบ
...
def find_gcd3(a, b, c, d): #ห.ร.ม. โดยใช้วิธี Euclidean algorithm (แบบใช้ modulus)
...
# ตัวอย่าง
gcd1, ops1 = find_gcd1(10, 15, 20 ,30)
gcd2, ops2 = find_gcd2(10, 15, 20 ,30)
gcd3, ops3 = find_gcd3(10, 15, 20 ,30)
